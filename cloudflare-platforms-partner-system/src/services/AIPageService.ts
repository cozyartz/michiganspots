    ...
    try {
      ...
      const aiResponse = await this.env.AI.run('@cf/meta/llama-3.1-8b-instruct' as any, {
        messages: [
          {
            role: 'system',
            content: 'You are an expert web developer and designer specializing in creating beautiful, branded business pages for Michigan Spots treasure hunt partners. Generate complete HTML pages that match the provided branding guidelines and showcase the business effectively. Always respond with valid HTML code.'
          },
          {
            role: 'user',
            content: prompt
