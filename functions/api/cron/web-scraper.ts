...
import { getTodaysTarget, type ScraperTarget } from './scraper-targets';

/**
 * Web Scraper for Michigan Businesses
 *
 * Scrapes public business directories to extract business data
 * NO EXTERNAL APIS - Pure HTML web scraping
 *
 * How to use:
 * 1. Add target websites to scraper-targets.ts
 * 2. Set enabled: true for targets you want to scrape
 * 3. Configure cron trigger: 0 3 * * * (daily at 3 AM UTC)
 * 4. Scraper rotates through enabled targets daily
 *
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // Import each business (limit to 20 per run to be respectful)
    const toImport = businesses.slice(0, 20);

    for (const business of toImport) {
      try {
        // Import via seed-business API
        const importResponse = await fetch(
          `${context.request.url.split('/api/')[0]}/api/directory/seed-business`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'x-seed-api-secret': env.SEED_API_SECRET || 'michigan-spots-seed-2025',
            },
...
/**
 * Extract businesses using CSS selectors
 * Works with Cloudflare's HTMLRewriter or manual parsing
 */
function extractByCSSSelectors(html: string, target: ScraperTarget): ScrapedBusiness[]  {
  const businesses: ScrapedBusiness[] = [];

  // Manual HTML parsing since HTMLRewriter needs Response object
  // We'll use regex to find elements matching the container selector

  // Convert CSS selector to regex pattern
  const containerClass = target.selectors.container.replace('.', '').replace('#', '');
  const containerRegex = new RegExp(
    `<(?:div|article|li|section)[^>]*(?:class|id)="[^"]*${containerClass}[^"]*"[^>]*>([\\s\\S]*?)</(?:div|article|li|section)>`,
    'gi'
  );
  ...
  for (const match of matches) {
    ...
    const name = extractBySelector(containerHtml, target.selectors.name);
    if (!name) continue;

    const address = target.selectors.address ? extractBySelector(containerHtml, target.selectors.address) : undefined;
    const phone = target.selectors.phone ? extractBySelector(containerHtml, target.selectors.phone) : undefined;
    const website = target.selectors.website
      ? extractLinkBySelector(containerHtml, target.selectors.website)
      : undefined;
    const description = target.selectors.description
      ? extractBySelector(containerHtml, target.selectors.description)
      : undefined;
 ...
 * Tries common HTML patterns for business listings
 */
function extractByRegexPatterns(html: string, target: ScraperTarget): ScrapedBusiness[] {
  ...
  const patterns = [
    // Pattern: Article tags with business info
    {
    ...
    // Pattern: Div with common business classes
    {
  ...
  for (const pattern of patterns) {
    ...
    // If we found businesses with this pattern, stop trying other patterns
